...
class ART:
    pngX = "art.png"
    name = str(input("What should call art?"))
    if f"{platform.system()[:3]}" == "Win":
        print(f"U are {platform.system()} user")
        locationX = str('C:\\ARTs\\')
        location_name = locationX + 'ARTS' + "\\" + name + "\\" + "backup" + "\\"
    else:
        print(f"U are {platform.system()} user")
        locationX = str('/storage/emulated/0/ART/')
        location_name = locationX + 'ARTS' + "/" + name + "/"  + "backup" + "/"
        #pngX = 'art_denoise.png'
    try:
        location_png = locationX + pngX
        imgX = Image.open(location_png)
        #print("png-file not found")
    except:
        location_png = str(input("enter png-file path"))
        #print(locationY)
        imgX = Image.open(location_png)
        #imgX.show()
    oldORnew = 1 # 1 == new # 0 == old #
    try:
        os.makedirs(location_name)
    except OSError:
        print('The directory( %s ) could not be created.' % location_name[:-7])
        exit()
        #   C:\backup\ARTs\1art.png

    print("File will be located on this path %s " % location_name[:-7])
    shutil.copyfile(location_png, location_name[:-7] + name + '.png')
    ...
    def g(self):
        ...
        def continueor():
            ...
            if poop == ("y" or "Y" or "Yes" or "YES" or "yes" or "yEs"):
                lists = func()
            else:
                lists = fonc()
            return lists
